...
export class GeminiService {
    ...
    async handleChat(req, res) {
        try {
            ...
            this.streamingEventService.setupStreamingResponse(res);
            // 发送错误事件而不是标准HTTP响应
            const errorCode = error instanceof Error && error.code ? error.code : ErrorCode.INTERNAL_ERROR;
            const errorMessage = error instanceof Error && error.code && Object.values(ErrorCode).includes(errorCode)
    ...
    async handleToolConfirmation(req, res) {
        try {
            ...
            if (!outcome || !Object.values(ToolConfirmationOutcome).includes(outcome)) {
            ...
            await this.toolOrchestrator.handleToolConfirmation(callId, outcome, abortController.signal);
    ...
    async handleModelStatus(req, res) {
        try {
            ...
            // 检查所有模型的可用性
            const modelStatuses = await Promise.all(supportedModels.map(async (model) => {
    ...
    async handleModelSwitch(req, res) {
        try {
            ...
            if (!SUPPORTED_MODELS.includes(model)) {
                return res.status(400).json(ResponseFactory.validationError('model', `Invalid model name. Valid models: ${SUPPORTED_MODELS.join(', ')}`));
            }
            // 切换模型
     ...
     * 检查模型可用性
     */
    async checkModelAvailability(model, authService) {
        try {
            // Flash模型默认可用
