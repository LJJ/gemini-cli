...
export class AuthService {
    ...
    currentApiKey = null;
    currentGoogleCloudProject = null;
    currentGoogleCloudLocation = null;
    ...
    async reconfigure(config) {
        ...
        try {
            ...
            this.updateStatus(ServiceStatus.ERROR, '重新配置失败', error);
            throw error;
        }
    }
    /**
    ...
    async initializeAuthState() {
        try {
            ...
            this.updateStatus(ServiceStatus.ERROR, '初始化认证状态失败', error);
            // 初始化失败不应该阻止服务启动
        }
    }
    /**
    ...
    async handleAuthConfig(req, res) {
        try {
        ...
        try {
            ...
            try {
                ...
                if (oauthError instanceof Error && this.oauthManager.isNetworkError(oauthError)) {
                    res.status(500).json(ResponseFactory.errorWithCode(ErrorCode.NETWORK_ERROR, '网络连接超时或缓存凭据过期，已自动清理缓存。请检查网络连接后重试。'));
                }
                else {
                    res.status(500).json(ResponseFactory.errorWithCode(ErrorCode.AUTH_CONFIG_FAILED, 'Google 登录失败，请检查网络连接或尝试使用 API Key 认证方式'));
                }
            }
        }
        catch (error) {
            console.error('Error in handleGoogleLogin:', error);
            res.status(500).json(ResponseFactory.internalError(error instanceof Error ? error.message : '启动 Google 登录失败'));
        }
    }
    ...
    async getContentGeneratorConfig(disableCodeAssist = false) {
        ...
        const config = await createContentGeneratorConfig(currentModel, this.currentAuthType);
